...
function icsFor(events: EventItem[]) {
  ...
  const lines = [
    ...
    "VERSION:2.0",
    "CALSCALE:GREGORIAN",
    "PRODID:-//CLPlanning//FR",
  ];
  const dt = (d: string, t: string) => {
    const z = new Date(`${d}T${t}:00`);
    return `${z.getUTCFullYear()}${String(z.getUTCMonth() + 1).padStart(2, "0")}${String(z.getUTCDate()).padStart(2, "0")}T${String(z.getUTCHours()).padStart(2, "0")}${String(z.getUTCMinutes()).padStart(2, "0")}00Z`;
  };
  for (const ev of events) {
    lines.push("BEGIN:VEVENT");
    lines.push(`UID:${ev.id}@clplanning`);
    lines.push(`DTSTAMP:${dt(ev.date, ev.start)}`);
  ...
  return (
    <main className="space-y-6 p-6">
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        <button
          onClick={exportICS}
          className="rounded-lg px-3 py-2 border bg-white shadow-sm"
        >
          Exporter ICS
        </button>
        <button
      ...
      <form
        onSubmit={addEvent}
        className="grid grid-cols-2 md:grid-cols-7 gap-3 p-3 border rounded-xl bg-white shadow-sm print:hidden"
      >
        <input
          name="title"
          placeholder="Titre (ex: Maths)"
          className="border rounded px-2 py-1 col-span-2 md:col-span-2"
          required
        />
        ...
        <button
          type="submit"
          className="col-span-2 md:col-span-1 rounded-lg px-3 py-2 bg-blue-600 text-white"
        >
          Ajouter
        </button>
      </form>
      ...
      <div className="grid" style={{ gridTemplateColumns: "5rem repeat(7, 1fr)" }}>
        ...
        {hours.map((hRow) => (
          <>
            <div
              key={`h-${hRow}`}
              className="border-t text-xs text-gray-600 p-2"
            >
              {String(hRow).padStart(2, "0")}:00
            </div>
            {week.map((d, i) => (
              <div key={`${d}-${hRow}`} className="relative border-t min-h-[3rem]">
                {events
                  .filter(
                    (ev) =>
                      ev.date === d &&
                      toMinutes(ev.start) < (hRow + 1) * 60 &&
                      toMinutes(ev.end) > hRow * 60
                  )
                  .map((ev) => {
                    const startM = toMinutes(ev.start);
                    const endM = toMinutes(ev.end);
                    const top = Math.max(0, (startM - hRow * 60) / 60 * 48);
                    const height = Math.max(
                      10,
                      Math.min(
                        60,
                        (Math.min((hRow + 1) * 60, endM) -
                          Math.max(hRow * 60, startM)) /
                          60 *
                          48
                      )
                    );
                    return (
                      <div
                        key={ev.id}
                        className="absolute left-1 right-1 rounded-md text-xs text-white p-1 shadow"
                        style={{
                          backgroundColor: ev.color || "#60a5fa",
                          top,
                          height,
                        }}
                      >
